...
global $valorGastoProjectos, $orcamentoProjectos, $orcamentoPDs, $listProgramas, $dataListvGasto, $dataListOrc, $dataListvObjesp, $dataListChart, $listaProg;
    ...
    switch ($_GET['report']) {
			...
			case "beneficiariosPDE":
				...
				$get_beneficiarios = $db->prepare("SELECT projecto.name as nome_projecto, fEtaria_bnf.value as fEtaria_bnf,
					...
					WHERE
						i.status_id = 1 and tracker_id = 11 and projecto.parent_id is not null and pde.parent_id = ? and fEtaria_bnf.value != ' ' and num_bnf.value != ' '  and fEtaria_bnf.value like '%Nr%' group by fEtaria_bnf, nome_projecto
						");
				...
				$get_beneficiariosMateria = $db->prepare("SELECT projecto.name as nome_projecto, variavel.value as variavel,
						COALESCE(SUM(case when num_bnf.value is null or num_bnf.value = '' then '0' else num_bnf.value end),0) as num_bnf
					FROM
						issues as i
						left join custom_values as num_bnf on (num_bnf.customized_id = i.id and num_bnf.custom_field_id = 105)
						left join custom_values as variavel on (variavel.customized_id = i.id and variavel.custom_field_id = 99)
						LEFT JOIN projects as projecto on (projecto.id = i.project_id)
                        LEFT JOIN projects as pde on (pde.id = projecto.parent_id)
					WHERE
						i.status_id = 1 and tracker_id = 11 and projecto.parent_id is not null and pde.parent_id = 12 and variavel.value != ' ' and num_bnf.value != ' '  and variavel.value not like '%Nr%' group by variavel, nome_projecto");
				$get_beneficiariosMateria->execute(array($id_report));

				$get_beneficiariosMaterias = $db->prepare("SELECT variavel.value as variavel,
						COALESCE(SUM(case when num_bnf.value is null or num_bnf.value = '' then '0' else num_bnf.value end)) as num_bnf
					FROM
						issues as i
						left join custom_values as num_bnf on (num_bnf.customized_id = i.id and num_bnf.custom_field_id = 105)
                        ...
                        LEFT JOIN projects as pde on (pde.id = projecto.parent_id)
					WHERE
						i.status_id = 1 and tracker_id = 11 and projecto.parent_id is not null and pde.parent_id = 12 and variavel.value != ' ' and num_bnf.value != ' '  and variavel.value not like '%Nr%' group by variavel");
				...
				if( $get_beneficiarios->rowCount() > 0){
					...
					$get_bnfHM_05 = $db->prepare("SELECT
						COALESCE(SUM(case when num_bnf_H5.value is null then 'NULL' else num_bnf_H5.value end), 0) as bnf
						FROM
							issues as i
							left join custom_values as num_bnf_H5 on (num_bnf_H5.customized_id = i.id and num_bnf_H5.custom_field_id = 105)
							left join custom_values as fEtaria_H5 on (fEtaria_H5.customized_id = i.id and fEtaria_H5.custom_field_id = 99 AND fEtaria_H5.value like '%0-5%')
							LEFT JOIN projects as projecto on (projecto.id = i.project_id)
							LEFT JOIN projects as pde on (pde.id = projecto.parent_id)
							WHERE
							i.status_id = 1
							...
							and pde.parent_id = ?
							and project_id != 12
							and fEtaria_H5.value != ' '
							and num_bnf_H5.value != ' '
							and fEtaria_H5.value like '%Nr%'
							");
